...
object CartItemRepository {
  ...
  val cartItemInsert = mysql.prepare("insert into CART_ITEM (CART_ID, POSITION, PRODUCT_ID, PRODUCT_OPTION) values (?, ?, ?, ?)")
  ...
  def insertToCart(cart: Cart, cartItem: CartItem) = {
    ...
    cartItemInsert.apply(toInsert.cartId, toInsert.position.get, toInsert.productId, toInsert.productOption).map {
      case OK(1L, _, _, _, _) =>
        toInsert
      case result =>
